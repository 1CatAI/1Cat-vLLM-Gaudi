...
def reference(dot, gate, beta):
    decay = torch.exp(gate.unsqueeze(-1) - gate.unsqueeze(-2)).to(beta.dtype)
    coefficient = beta.unsqueeze(-1) * decay
    lower = torch.tril(dot * coefficient, diagonal=-1)
    eye = torch.eye(CHUNK_SIZE, dtype=dot.dtype, device=dot.device)
    return lower + eye.unsqueeze(0)


def reference_bf16_gate(dot, gate, beta):
    return reference(dot, gate.to(torch.bfloat16).float(), beta)


def custom(dot, gate, beta):
    return torch.ops.custom_op.flashqla_kkt_form_bf16_gaudi2(
        dot,
...
def report_accuracy(actual, expected, label):
    ...
    print(
        f"{label}_max_abs={float(difference.max().cpu()):.9e} "
...
def main() -> None:
    ...
    compiled_reference_bf16_gate = torch.compile(
        reference_bf16_gate,
        **compile_args,
    )
    compiled_custom = torch.compile(custom, **compile_args)

    small_inputs = make_inputs(3)
    expected_small = compiled_reference_bf16_gate(*small_inputs)
    actual_small = compiled_custom(*small_inputs)
    torch.hpu.synchronize()
    report_accuracy(actual_small, expected_small, "small")
    ...
    print(f"shape=({BATCH},{CHUNK_SIZE},{CHUNK_SIZE})", flush=True)
    expected = benchmark(
        "reference",
        compiled_reference,
        inputs,
    ...
    indices = [0, 1, 31, 255, 1023, 4095, 8191, BATCH - 1]
    expected_bf16_gate = compiled_reference_bf16_gate(*inputs)
    torch.hpu.synchronize()
    report_accuracy(actual[indices], expected[indices], "sampled_original_gate")
    report_accuracy(
        actual[indices],
        expected_bf16_gate[indices],
        "sampled_bf16_gate",
    )
    torch.testing.assert_close(
        actual[indices].cpu(),
        expected_bf16_gate[indices].cpu(),
        rtol=0.02,
        atol=2.5e-4,
    )
