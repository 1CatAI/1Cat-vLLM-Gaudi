...
int main() {
    ...
    uint32_t count = 0;
    assert(GetKernelGuids(DEVICE_ID_GAUDI2, &count, nullptr) == GLUE_SUCCESS && count == 5);
    GuidInfo guids[5]{};
    count = 0;
    assert(GetKernelGuids(DEVICE_ID_GAUDI2, &count, guids) == GLUE_SUCCESS && count == 5);
    assert(guids[0].name[0] == '\0');
    count = 1;
    assert(GetKernelGuids(DEVICE_ID_GAUDI2, &count, guids) == GLUE_FAILED && count == 5);
    assert(guids[0].name[0] == '\0');
    count = 2;
    assert(GetKernelGuids(DEVICE_ID_GAUDI2, &count, guids) == GLUE_FAILED && count == 5);
