    ...
    for (int group = begin[2]; group < end[2]; ++group) {
        for (int token = begin[1]; token < end[1]; ++token) {
            ...
            for (int tile = begin[0]; tile < end[0]; ++tile) {
                ...
                float128 rounded = convert_bfloat128_to_float128(first, SW_LINEAR);
                rounded.v1 = v41_group32_roundtrip(rounded.v1);
                rounded.v2 = v41_group32_roundtrip(rounded.v2);
                const bfloat128 result = convert_float128_to_bfloat128(rounded, SW_RHNE | SW_LINEAR);
