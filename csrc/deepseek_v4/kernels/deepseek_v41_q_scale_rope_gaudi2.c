    ...
    for (int row = start[1]; row < end[1]; ++row) {
        ...
        for (int head = start[0]; head < end[0]; ++head) {
            for (int block = 0; block < 3; ++block) {
                ...
                float128 scaled;
                scaled.v1 = v_f32_ld_tnsr_b(at, product) * v_f32_ld_tnsr_b((int5){at[0], 0, 0, 0, 0}, weight_scale) * sx;
                at[0] += 64;
                scaled.v2 = v_f32_ld_tnsr_b(at, product) * v_f32_ld_tnsr_b((int5){at[0], 0, 0, 0, 0}, weight_scale) * sx;
                at[0] -= 64;
