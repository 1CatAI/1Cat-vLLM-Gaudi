...
extern unsigned char
    _binary___deepseek_v41_router_logits_top6_gaudi2_o_start;
extern unsigned char
    _binary___deepseek_v41_router_logits_top6_gaudi2_o_end;
tpc_lib_api::GlueCodeReturn
DeepseekV41RouterLogitsTop6Gaudi2::GetGcDefinitions(
    ...
    for (int i = 1; i < 4; ++i) {
        ...
        if (t.dims != 1 || t.maxSizes[0] != (i == 3 ? tokens : 384) ||
            t.dataType != (i == 3 ? DATA_I8 : DATA_F32))
            return GLUE_INCOMPATIBLE_INPUT_SIZE;
    }
    for (int i = 0; i < 2; ++i) {
        const auto& t = p->outputTensors[i].geometry;
        if (t.dims != 2 || t.maxSizes[0] != 6 ||
            t.maxSizes[1] != tokens ||
            t.dataType != (i == 0 ? DATA_I32 : DATA_F32))
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        t.mapping[0].indexSpaceDim = 0;
        t.mapping[0].a = i == 3 ? 1 : 0;
        t.mapping[0].end_b = i == 3 ? 0 : 383;
        if (i == 0) {
            t.mapping[1].indexSpaceDim = 0;
            t.mapping[1].a = 1;
        }
    }
    for (int i = 0; i < 2; ++i) {
