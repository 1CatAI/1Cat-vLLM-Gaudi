...
tpc_lib_api::GlueCodeReturn DeepseekV41WoaGaudi2::GetGcDefinitions(
    ...
    if (p->inputTensorNr != (quant_ ? (rope_ ? 3u : 1u) : 3u)) return GLUE_INCOMPATIBLE_INPUT_COUNT;
    ...
    if (quant_) {
        ...
        if (rope_) {
            ...
            const auto& phase = p->inputTensors[2].geometry;
            if (positions.dataType != DATA_I32 || positions.dims != 1 || positions.maxSizes[0] != tokens ||
                phase.dataType != DATA_F32 || phase.dims != 2 || phase.maxSizes[0] != 64 || !phase.maxSizes[1])
                return GLUE_INCOMPATIBLE_INPUT_SIZE;
        }
