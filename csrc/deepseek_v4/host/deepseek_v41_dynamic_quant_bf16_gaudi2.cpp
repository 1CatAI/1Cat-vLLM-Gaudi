...
extern unsigned char _binary___deepseek_v41_dynamic_quant_bf16_gaudi2_o_start;
extern unsigned char _binary___deepseek_v41_dynamic_quant_bf16_gaudi2_o_end;

tpc_lib_api::GlueCodeReturn DeepseekV41DynamicQuantBf16Gaudi2::GetGcDefinitions(
    ...
    // The original C1 route used at most six experts and therefore exposed a
    // 36-row contract (6 tokens x top-6).  The same quantizer is now used by
    // the normal vLLM chunked-prefill path: the non-broadcast W2 input has
    // ``tokens * top6`` rows, up to 8192*6.  One index-space point still owns
    // one row, so the TPC body does not need a different algorithm; only the
    // host shape contract must stop rejecting valid prefill tiles.
    if (input.dims != 2 || output.dims != 2 || scale.dims != 2 || rows < 1 || rows > 49152 ||
    ...
    const unsigned size = &_binary___deepseek_v41_dynamic_quant_bf16_gaudi2_o_end - &_binary___deepseek_v41_dynamic_quant_bf16_gaudi2_o_start;
    ...
    std::memcpy(instance->kernel.kernelElf, &_binary___deepseek_v41_dynamic_quant_bf16_gaudi2_o_start, size);
    return GLUE_SUCCESS;
}
