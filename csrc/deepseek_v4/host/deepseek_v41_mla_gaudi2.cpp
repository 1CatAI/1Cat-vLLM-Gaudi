...
tpc_lib_api::GlueCodeReturn DeepseekV41MlaGaudi2::GetGcDefinitions(
    ...
    const unsigned inputs = gather_ ? 6 : 4, outputs = gather_ ? 3 : 1;
    ...
    if (gather_) {
        ...
        if (!matches(in->outputTensors[0], DATA_BF16, 2, 512) ||
            !matches(in->outputTensors[1], DATA_F32, 2, 512) ||
            !matches(in->outputTensors[2], DATA_F32, 1, width) ||
        ...
        if (!matches(in->outputTensors[0], DATA_F32, 2, width) ||
