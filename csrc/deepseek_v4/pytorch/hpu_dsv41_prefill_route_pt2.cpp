...
namespace {
constexpr auto kWrite="custom_op::custom_deepseek_v41_prefill_route_write_gaudi2";
constexpr auto kOrdered="custom_op::custom_deepseek_v41_prefill_route_ordered_gaudi2";
constexpr auto kRead="custom_op::custom_deepseek_v41_prefill_route_read_gaudi2";
...
habana::PartialOutputMetaDataVector metadata(const at::Stack& s,bool read) {
    ...
    TORCH_CHECK((cache.scalar_type()==at::kByte||cache.scalar_type()==at::kBFloat16||cache.scalar_type()==at::kFloat)&&
                cache.dim()==2&&cache.size(0)>0&&cache.size(1)>0&&cache.size(1)<=32768&&
                rows.scalar_type()==at::kInt&&rows.dim()==1&&rows.numel()>0&&rows.numel()<=4096,
                "State rows require bounded prefill route rows and supported state dtype");
...
TORCH_LIBRARY_FRAGMENT(custom_op,m) {
    m.def("custom_deepseek_v41_prefill_route_write_gaudi2(Tensor(a!) cache, Tensor value, Tensor rows) -> Tensor");
    m.def("custom_deepseek_v41_prefill_route_ordered_gaudi2(Tensor cache, Tensor value, Tensor rows) -> Tensor");
    m.def("custom_deepseek_v41_prefill_route_read_gaudi2(Tensor cache, Tensor rows, Tensor completion) -> Tensor");

}
TORCH_LIBRARY_IMPL(custom_op,HPU,m) {
    m.impl("custom_deepseek_v41_prefill_route_write_gaudi2",execute<false,false>);
    m.impl("custom_deepseek_v41_prefill_route_ordered_gaudi2",execute<false,false>);
    m.impl("custom_deepseek_v41_prefill_route_read_gaudi2",execute<false,true>);

}
TORCH_LIBRARY_IMPL(custom_op,Meta,m) {
    m.impl("custom_deepseek_v41_prefill_route_write_gaudi2",execute<true,false>);
    m.impl("custom_deepseek_v41_prefill_route_ordered_gaudi2",execute<true,false>);
    m.impl("custom_deepseek_v41_prefill_route_read_gaudi2",execute<true,true>);

}
TORCH_LIBRARY_IMPL(custom_op,Functionalize,m) {
    m.impl("custom_deepseek_v41_prefill_route_write_gaudi2",functionalize);
}
