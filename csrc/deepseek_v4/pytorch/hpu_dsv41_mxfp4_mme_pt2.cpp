...
class PreparedV41 final : public habana::OpBackend {
        ...
        if (fp8_) {
            auto decoded = n256_
                ? BuildNode(this, graph, {kN256Fp8, {ids, q, s, lookup},
                    {{{experts, k, n}, at::ScalarType::Float8_e4m3fn}}})
            ...
            if (n256_) {
                auto scaled = BuildNode(this, graph, {kN256Scale,
                    {product.at(0).get(), ids, quant.at(1).get(), channel}, {{resultShape, at::kBFloat16}}});
                return ReshapeHelper(graph, scaled.at(0).get(), {1, experts, n}, at::kBFloat16);
            }
    ...
    Tensor fused_down(synapse_helpers::graph& graph, synTensor ids, int experts, int hidden, int intermediate) {
        auto quant = BuildNode(this, graph, {kDynamicQuant, {syn_in(0)},
            {{{1, hidden}, at::ScalarType::Float8_e4m3fn}, {{1, 1}, at::kFloat}}});
        auto x = ReshapeHelper(graph, quant.at(0).get(), {1, 1, hidden}, at::ScalarType::Float8_e4m3fn);
        auto w13 = BuildNode(this, graph, {kN256Fp8, {ids, syn_in(3), syn_in(5), syn_in(7)},
            {{{experts, hidden, intermediate * 2}, at::ScalarType::Float8_e4m3fn}}});
        synGEMMParams params{false, false};
        auto p13 = BuildNode(this, graph, {"batch_gemm", {x.get(), w13.at(0).get()},
            {{{experts, 1, intermediate * 2}, at::kFloat}}, &params, sizeof(params)});
        auto middle = BuildNode(this, graph, {kN256SiluQuant,
            {p13.at(0).get(), ids, quant.at(1).get(), syn_in(8), syn_in(2)},
            {{{experts, 1, intermediate}, at::ScalarType::Float8_e4m3fn}, {{experts, 1, 1}, at::kFloat}}});
        auto w2 = BuildNode(this, graph, {kN256Fp8, {ids, syn_in(4), syn_in(6), syn_in(7)},
            {{{experts, intermediate, hidden}, at::ScalarType::Float8_e4m3fn}}});
        auto p2 = BuildNode(this, graph, {"batch_gemm", {middle.at(0).get(), w2.at(0).get()},
            {{{experts, 1, hidden}, at::kFloat}}, &params, sizeof(params)});
        auto sx2 = ReshapeHelper(graph, middle.at(1).get(), {experts, 1}, at::kFloat);
        auto scaled = BuildNode(this, graph, {kN256Scale, {p2.at(0).get(), ids, sx2.get(), syn_in(9)},
            {{{experts, 1, hidden}, at::kBFloat16}}});
        return ReshapeHelper(graph, scaled.at(0).get(), {1, experts, hidden}, at::kBFloat16);
    }
 ...
 public:
    ...
    void AddNode(synapse_helpers::graph& graph, const at::Stack& stack) override {
        ...
        auto down = [&]() -> Tensor {
            if (fused_) return fused_down(graph, ids.get(), experts, hidden, intermediate);
