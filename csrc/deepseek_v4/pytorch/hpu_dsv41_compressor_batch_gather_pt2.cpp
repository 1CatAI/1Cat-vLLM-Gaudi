...
constexpr auto kPair =
    "custom_op::custom_deepseek_v41_compressor_batch_gather_f32_gaudi2";
constexpr auto kPairOrdered =
    "custom_op::custom_deepseek_v41_compressor_batch_gather_ordered_f32_gaudi2";
...
const bool registered = [] {
    for (auto name : {kPair, kPairOrdered}) {
        habana::custom_op::registerUserCustomOp(
            name, "custom_deepseek_v41_compressor_batch_gather_f32_gaudi2",
            [](const at::Stack& stack) {
                ...
                return habana::PartialOutputMetaDataVector{
                    {at::kFloat, {stack.at(2).toTensor().size(0), 4, 512}}};
            }, nullptr);
    }
    return true;
}();
...
at::Tensor pair(const at::Tensor& kv_history,
    ...
    if constexpr (Meta)
        return at::empty({kv.size(0), 4, 512}, kv.options());
...
TORCH_LIBRARY_FRAGMENT(custom_op, m) {
    m.def("custom_deepseek_v41_compressor_batch_gather_f32_gaudi2(Tensor(a!) kv_history, Tensor(b!) score_history, Tensor kv, Tensor score, Tensor position, Tensor slots) -> Tensor");
    m.def("custom_deepseek_v41_compressor_batch_gather_ordered_f32_gaudi2(Tensor kv_history, Tensor score_history, Tensor kv, Tensor score, Tensor position, Tensor slots) -> Tensor");
}
TORCH_LIBRARY_IMPL(custom_op, HPU, m) {
    m.impl("custom_deepseek_v41_compressor_batch_gather_f32_gaudi2",
           pair<false, false>);
    m.impl("custom_deepseek_v41_compressor_batch_gather_ordered_f32_gaudi2",
           pair<false, true>);
}
...
TORCH_LIBRARY_IMPL(custom_op, Functionalize, m) {
    m.impl("custom_deepseek_v41_compressor_batch_gather_f32_gaudi2", functionalize);
}
