...
namespace {
constexpr auto schema="custom_op::custom_deepseek_v41_index_keys_gaudi2";
...
habana::PartialOutputMetaDataVector metadata(const at::Stack& s) {
    ...
    TORCH_CHECK((ratio==1||ratio==2)&&cache.scalar_type()==at::kByte&&cache.dim()==2&&cache.size(1)==68&&
                cache.size(0)>0&&cache.size(0)<=1048704&&pages.scalar_type()==at::kInt&&pages.dim()==1&&
                pages.numel()>0&&pages.numel()<=8192&&rows.scalar_type()==at::kInt&&rows.dim()==2&&
...
template<bool Fake> at::Tensor gather(const at::Tensor& cache,const at::Tensor& pages,const at::Tensor& rows,int64_t ratio) {
...
TORCH_LIBRARY_IMPL(custom_op,HPU,m) {m.impl("custom_deepseek_v41_index_keys_gaudi2",gather<false>);}
TORCH_LIBRARY_IMPL(custom_op,Meta,m) {m.impl("custom_deepseek_v41_index_keys_gaudi2",gather<true>);}
