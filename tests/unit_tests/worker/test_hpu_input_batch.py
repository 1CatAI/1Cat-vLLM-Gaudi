...
def _remove_requests(input_batch: InputBatch, batch_size: int,
    ...
    return req_ids_to_remove, req_indices_to_remove
...
def _construct_expected_sampling_metadata(
    ...
    return SamplingMetadata(
        ...
        no_penalties=(all(x == 0 for x in presence_penalties) and all(x == 0 for x in frequency_penalties)
...
def test_sampling_metadata_in_input_batch(device: str, batch_size: int):
    ...
    req_ids_to_remove, req_indices_to_remove = _remove_requests(input_batch, batch_size, reqs)
    ...
    expected_sampling_metadata = _construct_expected_sampling_metadata(reqs,
                                                                       req_ids_retained,
