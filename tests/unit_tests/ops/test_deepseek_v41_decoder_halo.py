...
def test_halo_uses_aligned_prefix_blocks_across_prompt_lengths():
    ...
    assert [decoder_halo_mode(start, n, 20480, eligible=True)
            for start, n in ((0, count), (count, count), (2 * count, 4096))] == ["prefix_only", "prefix_only", "final"]
    assert [decoder_halo_mode(start, n, 20000, eligible=True)
            for start, n in ((0, count), (count, count), (2 * count, 3616))] == ["prefix_only", "full", "final"]
    assert [decoder_halo_mode(start, n, 32769, eligible=True)
            for start, n in ((0, count), (count, count), (2 * count, count), (3 * count, count),
                             (4 * count, 1))] == ["prefix_only", "prefix_only", "prefix_only", "full", "final"]
...
@pytest.mark.parametrize("start,count,prompt_tokens", [(-1, 8192, 32768), (0, 0, 32768),
                                                          (24576, 8192, 32767)])
