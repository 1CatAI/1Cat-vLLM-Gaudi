...
def test_runtime_selector_publishes_current_state_without_rebinding(monkeypatch, layer, ratio):
    ...
    attention.shared = SimpleNamespace(candidate_pool=pool, block_table=torch.ones(8192, dtype=torch.int32),
    ...
    def select(q, w, cache, pages, positions, candidates, **options):
        ...
        assert options == dict(ratio=ratio, capacity=(1 << 20) // ratio,
                               reindex=layer > 20, publish_candidates=layer == 20)
