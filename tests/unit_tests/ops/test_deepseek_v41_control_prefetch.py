...
"""Control load lookahead preserves the ordered FP32 dot product."""
import json
import os
...
def test_control_prefetch_exact_with_tail_zero_and_reordered_requests(batch):
    torch._dynamo.reset()
    ...
    reference = torch.compile(torch.ops.custom_op.custom_deepseek_v41_control_gemv_f32_gaudi2,
                              ...
                              fullgraph=True,
                              dynamic=False)
    candidate = torch.compile(torch.ops.custom_op.custom_deepseek_v41_control_prefetch_f32_gaudi2,
                              backend="hpu_backend",
                              fullgraph=True,
                              dynamic=False)
...
def test_control_prefetch_rejects_wrong_width():
    candidate = torch.ops.custom_op.custom_deepseek_v41_control_prefetch_f32_gaudi2
