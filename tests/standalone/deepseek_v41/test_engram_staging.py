...
def test_staging_views_preserve_all_bytes_and_capacity(heads):
    ...
    for count in (1, 6, 1, 2, 1):
        ...
        expected = np.concatenate((weights[:count * heads].reshape(count, heads, 256),
                                   scales[:count * heads].reshape(count, heads, 8)), axis=-1)
        ...
        for request in ("first", "second"):
            ...
            for step, count in enumerate((1, 6, 1, 2, 1, 6, 1, 1, 1)):
                ...
                with torch.hpu.stream(consumer):
                    ticket = host.prepare(request, tokens, [i == 1 for i in range(count)])
