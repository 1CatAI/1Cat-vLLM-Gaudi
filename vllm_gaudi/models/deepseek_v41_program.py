...
from vllm_gaudi.ops.deepseek_v41_math import (
    engram_update, hc_post, hc_pre, quantize_activation, rms_norm, unpack_swa,
)
...
def _weight_tree(specs):
    root = nn.Module()
    dtypes = {"BF16": torch.bfloat16, "F32": torch.float32, "U8": torch.uint8,
              "I8": torch.int8, "I16": torch.int16, "I32": torch.int32,
              "I64": torch.int64, "F8_E4M3": torch.bfloat16}
...
def load_weight_tree(shard, tree, device, specs=None):
    ...
    for name, spec in (shard.specs if specs is None else specs).items():
        if n256 and name.startswith("layers.") and ".ffn.experts." in name:
        ...
        if (gaudi_envs.VLLM_HPU_DSV41_PRETRANSPOSE_ATTN
                and name.endswith(".wo_a.weight") and value.ndim == 2):
            if value.shape[0] % 4:
        ...
        if (name.endswith("ffn.gate.weight") or name.endswith("confidence_head.proj.weight")
                ...
                or (".compressor.w" in name and ".weight" in name
                    and int(name.split(".")[1]) in (2, 8, 14))):
            value = value.float()
        module_name, _, attribute = name.rpartition(".")
...
class PreparedMoE(nn.Module):
    def __init__(self, weights, topk, normal_scales, lookup, reduce):
        ...
        self.n256 = topk == 6 and (gaudi_envs.VLLM_HPU_DSV41_EXPERT_N256
                                  or gaudi_envs.VLLM_HPU_DSV41_EXPERT_N256_FP8)
    ...
    def _can_use_indexed(self, value, ids, routing):
        ...
        return (
            ...
            and self.topk == 6
            and value.device.type == "hpu" and value.dtype == torch.bfloat16
            and value.ndim == 2 and 1 <= value.shape[0] <= 6 and value.shape[-1] == 5120
            and ids.device == value.device and ids.dtype == torch.int64
            and ids.ndim == 2 and ids.shape[-1] == 6
            and routing.device == value.device and routing.dtype == torch.float32
    ...
    def _forward_indexed(self, value, ids, routing):
        ...
        gate_up = torch.ops.custom_op.custom_deepseek_v41_mxfp4_indexed_fc1_bf16_gaudi2(
            value, ids.to(torch.int32), experts.w13_q16, experts.w13_s16,
            self.lookup, self.normal_scales)
        ...
        down = torch.ops.custom_op.custom_deepseek_v41_mxfp4_indexed_fc2_bf16_gaudi2(
            routed_input, ids.to(torch.int32), experts.w2_q16, experts.w2_s16,
            self.lookup, self.normal_scales)
        result = down.float()[:, 0]
    ...
    def forward(self, value, image_mask, ready_outputs=(), *, fp8_decode=False):
        ...
        if self.n256:
        ...
        elif (gaudi_envs.VLLM_HPU_DSV41_SHARED_C6_EXPERTS and self.topk == 6
                and value.device.type == "hpu" and 2 <= value.shape[0] <= 6):
            ...
            if (gaudi_envs.VLLM_HPU_DSV41_TILED_EXPERT_DECODE and self.topk == 6
                    and value.device.type == "hpu" and 2 <= value.shape[0] <= 6):
                op = torch.ops.custom_op.custom_deepseek_v41_mxfp4_k128_moe_bf16_gaudi2
            if (gaudi_envs.VLLM_HPU_DSV41_W13_N512 and self.topk == 6
                    and value.device.type == "hpu" and 2 <= value.shape[0] <= 6):
...
class PreparedDecoderLayer(nn.Module):
    def __init__(self, weights, config, layer, shared, normal_scales, lookup, reduce, all_gather, device):
        super().__init__()
    ...
    def forward(self, residual, pre_mix, positions, image_mask, engram_rows=None, *, fp8_decode=False):
        ...
        target_state = residual.mean(1) if self.layer in (37, 38, 39) else None
        value, new_pre, post, comb = hc_pre(residual, pre_mix, w.hc_attn_fn, w.hc_attn_scale, w.hc_attn_base,
                                           self.eps, self.hc_eps, self.iterations)
        value = rms_norm(value, w.attn_norm.weight, self.eps)
        schedule = (gaudi_envs.VLLM_HPU_DSV41_MHC_SCHEDULE
                    and not self.draft and 1 <= value.shape[0] <= 6)
        if schedule:
        ...
        value, pre_mix, post, comb = hc_pre(residual, new_pre, w.hc_ffn_fn, w.hc_ffn_scale, w.hc_ffn_base,
                                          self.eps, self.hc_eps, self.iterations)
        value = self.moe(rms_norm(value, w.ffn_norm.weight, self.eps), image_mask,
                         ready_outputs=(post, comb) if schedule else (), fp8_decode=fp8_decode)
...
class PreparedStage(nn.Module):
    def __init__(self, directory, pp_rank, tp_rank, reduce, all_gather, device, max_length=512):
        ...
        self.weights = _weight_tree(self.shard.specs)
        ...
        for layer in range(self.start, self.stop):
            ...
            self.layers.append(PreparedDecoderLayer(self.weights.layers.get_submodule(str(layer)), config, layer,
                               self.shared, normal, lookup, reduce, all_gather, device))
        self.generation, self.loaded = 0, False
        self.runtime_precision = {"experts": "MXFP4 -> BF16 SRAM -> BF16 MME",
                                  ...
                                  "mHC_router": "FP32", "communication": "BF16"}
        self.draft = PreparedDraft(self, lookup, device) if pp_rank == 1 else None

    def load_prepared(self, device):
        if self.loaded:
            self.invalidate()
        load_weight_tree(self.shard, self.weights, device)
        for layer in self.layers:
            layer.attention.prepare_qkv_input_weight()
        if gaudi_envs.VLLM_HPU_DSV41_EXPERT_N256 or gaudi_envs.VLLM_HPU_DSV41_EXPERT_N256_FP8:
            from vllm_gaudi.ops.deepseek_v41_expert_n256 import FINGERPRINT
            ...
            self.runtime_precision["experts"] = (
                "MXFP4 -> FP8 SRAM -> FP8xFP8 MME for native C1-C6; BF16 prefill"
                if gaudi_envs.VLLM_HPU_DSV41_EXPERT_N256_FP8
                else "N256 MXFP4 -> BF16 SRAM -> BF16 MME")
    ...
    def invalidate(self):
        ...
        for layer in self.layers:
            layer.attention.invalidate_qkv_input_weight()
        self.loaded = False
        self.generation += 1
    ...
    def forward(self, residual, pre_mix, positions, input_ids, engram_rows):
        ...
        return value, pre_mix, torch.cat(target_states, -1)
...
class PreparedLayerGroup(nn.Module):
    ...
    def __init__(self, stage, start, stop, *, pp_wire_input=False, fused_text_io=False):
        ...
        self.tp_rank, self.reduce = stage.tp_rank, stage.reduce
    ...
    def native_forward(self, residual, pre_mix, positions, input_ids, engram_rows):
        return self._forward(residual, pre_mix, positions, input_ids, engram_rows, fp8_decode=True)
...
def _compile_group(group, *, native, backend="hpu_backend"):
    ...
    entry = FunctionType(function.__code__.replace(co_name=name), function.__globals__,
...
class CompiledStage:
    def __init__(self, stage, *, native=False, pp_wire_input=False, fused_text_io=False):
        self.groups = tuple(PreparedLayerGroup(stage, start, start + 4, pp_wire_input=pp_wire_input,
...
class PreparedDraft(nn.Module):
    ...
    def __init__(self, stage, lookup, device):
        ...
        for index in range(3):
            ...
            self.layers.append(PreparedDecoderLayer(self.weights.get_submodule(str(index)), config, index + 40,
                               stage.shared, normal, lookup, self.reduce, self.all_gather, device))
    ...
    def _sample_greedy(self, first_token, hidden, logits, *, local):
        ...
        for index in range(5):
            ...
            previous = (self._global_argmax(scores_logits) if local
                        else scores_logits.argmax(dim=-1))
    ...
    def verify_prefix(self, target_hidden, proposed_ids, metadata, target_states,
                      target_positions):
        ...
        from vllm_gaudi.ops.deepseek_v41_verify import (
            encode_record_wire, pack_record, verify_control_from_target,
        )
        ...
        commit_record = pack_record(metadata, committed, output_count, output,
                                    placeholder, torch.zeros_like(draft_enabled), status)
        return (target, output, committed, output_count, anchor, draft_enabled, status,
                commit_record, encode_record_wire(commit_record))

    def draft_from_prefix(self, metadata, target_positions, output, committed,
                          output_count, anchor, draft_enabled, status):
        ...
        record = pack_record(metadata, committed, output_count, output, draft_ids,
                             draft_enabled, status)
        return record, encode_record_wire(record), confidence

    def verify_and_propose(self, target_hidden, proposed_ids, metadata, target_states,
                           target_positions):
