...
def prepare_native_libraries():
    ...
    if configured_library:
        ...
        if os.environ.get("VLLM_HPU_DSV41_ENGRAM_HOST_TABLE") == "1":
            host = list(library_dir.glob("dsv41_host_gather*.so"))
            host_manifest = json.loads((library_dir / "deepseek_v41_build.json").read_text())
            if len(host) != 1 or hashlib.sha256(host[0].read_bytes()).hexdigest() != host_manifest["binaries"].get(
                    host[0].name):
                raise RuntimeError("V4.1 Engram host binary is missing or differs from its build manifest")
