...
def locked_file_reuse(owners, *, proc_root=Path("/proc")):
            ...
            for line in smaps.splitlines():
                ...
                if match:
                    ...
                    if key in wanted and match[3] == "r--s":
                        current = dict(key=key, offset=int(match[4], 16),
                                       length=int(match[2], 16) - int(match[1], 16), rss=0)
                    ...
                    if "lo" in line.split()[1:] and current["rss"] == current["length"]:
                        ...
                        for start, stop in wanted[current["key"]]:
                            ...
                            if overlap[1] > overlap[0]:
                                ...
                                records.append(dict(pid=int(process.name), device_inode=current["key"],
                                                    offset=overlap[0], length=overlap[1] - overlap[0]))
                    current = None
    ...
    return dict(reused_bytes=reused, mappings=records,
                source_identities={file: dict(zip(("device", "inode", "size", "mtime_ns"), identity))
                                   for file, identity in identities.items()})
...
class EngramResidency:
    ...
    def start(self, *, timeout=600, check_available=True):
        ...
        if check_available:
            ...
            self.admission = dict(audit, available_bytes=available, required_available_bytes=required,
                                  budget_bytes=self.budget_bytes, mapped_bytes=requested)
            if available < required:
                raise RuntimeError(
                    f"Engram host budget unavailable after locked-page reuse: {available} < {required}; "
