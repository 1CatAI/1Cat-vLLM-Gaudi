...
def permuted_body_write(value, routing, slots, expert_ids, q13, q2, s13, s2, lookup, normal_scales,
                        routed, indices):
    ...
    weight13 = torch.ops.custom_op.custom_deepseek_v41_prefill_permuted_bf16_gaudi2(
        local_experts, q13, s13, lookup)
    ...
    middle = torch.ops.custom_op.custom_deepseek_v41_bf16_identity_gaudi2(middle.reshape(1, -1)).reshape(
        groups, rows, width)
    weight2 = torch.ops.custom_op.custom_deepseek_v41_prefill_permuted_bf16_gaudi2(
        local_experts, q2, s2, lookup)
