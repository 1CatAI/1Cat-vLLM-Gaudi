...
INDEX_KEY_TILE = 2048
_index_tp_audit = {"full_calls": 0, "reindex_calls": 0, "input_queries": 0, "local_queries": 0,
...
def full_prefill_index_selection(query, weights, packed, table, positions, ratio, source_rows,
                                 publish_candidates, visible_rows=None):
...
def tp_full_prefill_index_selection(query, weights, packed, table, positions, ratio, source_rows,
                                    publish_candidates, tp_rank, all_gather, visible_rows=None):
    ...
    selected, blocks = full_prefill_index_selection(local_q, local_w, packed, table, local_p, ratio,
                                                   source_rows, publish_candidates, visible_rows)
...
def prefill_reindex_selection(query, weights, packed, table, positions, blocks, ratio, source_rows,
                              native_gather=True, native_scores=True):
    ...
    for start in range(0, query.shape[0], 128):
        ...
        outputs.append(compiled_decoded_reindex(signature)(query[start:stop].clone(), weights[start:stop].clone(),
                                                           keys, positions[start:stop].clone(), current, ratio, 16,
                                                           native_gather, native_scores))
    return torch.cat(outputs, 0)


def tp_prefill_reindex_selection(query, weights, packed, table, positions, blocks, ratio, source_rows,
                                 tp_rank, all_gather, native_gather=True, native_scores=True):
