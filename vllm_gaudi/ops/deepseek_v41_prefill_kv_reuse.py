...
def _decode_compact(packed, table, logical, ratio):
    ...
    return torch.ops.custom_op.custom_deepseek_v41_prefill_main_decode_gaudi2(
        compressed_rows, table, logical, 0)
    ...
    def get(self, source, packed, table, logical, ratio, *, producer=False):
        ...
        if producer or cached is None or cached[0] != key:
            if gaudi_envs.VLLM_HPU_DSV41_PREFILL_MAIN_DECODE:
                ...
                value = _compiled(_decode_compact, _signature(packed, table, logical, ratio))(
                    packed, table, logical, ratio)
            else:
