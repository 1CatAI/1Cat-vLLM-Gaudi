...
| `VLLM_HPU_QWEN3_COMPILE_LAYER_GROUP_SIZE` | Compiles consecutive Qwen3-Next/Qwen3.5 decoder layers as one regional graph during decode. Prefill remains per-layer. Values greater than 1 require TP=1 and non-eager execution. Decode batches above 16 automatically use the per-layer compiled path to preserve output correctness. | `1` |
