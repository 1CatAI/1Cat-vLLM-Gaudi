...
| `VLLM_HPU_GDN_ACTIVE_CONV_STATE_VIEWS` | Experimental: binds the active BF16 convolution history before TP2 single-token decoder compilation, avoiding shared-pool materialization across communication boundaries. Requires active recurrent-state views and the compatible native runtime; unsupported decode modes retain their existing cache paths. | `0` |
| `VLLM_HPU_TP2_GQA_NATIVE_MATMUL` | Experimental: sends compact BF16 GQA directly to native batch GEMM nodes and expresses QK transposition in the MME descriptor. Requires Gaudi2 TP2 C1 compact attention and a bridge built with the native GQA operator. | `0` |
...
| `VLLM_HPU_TP2_PREPARED_CONV_TAPS` | Research TP2 C1 direct decode: prepare four contiguous FP32 convolution taps through the checkpoint loader, preserving BF16 convolution arithmetic and state updates. Refreshes through weight loading or `load_state_dict`; requires rebuilding after allocation or dtype changes. | `0` |
| `VLLM_HPU_TP2_SINGLE_BATCH_MAPPING` | Research TP2 C1 compact GQA: express one-column BF16 batch-to-block mapping as a broadcast product. Requires compact GQA; block-to-batch reductions and all TP collectives remain intact. | `0` |
| `VLLM_HPU_TP2_PREPARED_GEMMA_WEIGHT` | Research TP2 single-token decode: prepare the BF16 Gemma norm weight offset during normal loading. Native plans retire before reloading. Requires the qualified dense width and direct-state layout. | `0` |
| `VLLM_HPU_TP2_MLP_SPLIT_SCALE` | Research grouped Gaudi2 TP2 decode: split gate/up before FP32 scaling and BF16 rounding so the compiler can combine scaling with the existing SiLU nodes. Requires loaded channel FP8 weights, native dynamic quantization and the qualified dense geometry. Prefill uses the original MLP. | `0` |
| `VLLM_HPU_TP2_PREPARED_MROPE` | Research Gaudi2 TP2 single-token decode: prepare a fixed MRoPE channel map and reuse the shared rotary coefficients from the compiled model input preparation. Keeps all three position axes dynamic. Requires one shared interleaved rotary module and fixed native input bindings. | `0` |
