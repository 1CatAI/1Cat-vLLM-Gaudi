...
| `VLLM_HPU_DSV41_PREFILL_NATIVE_PLAN` | Replays fixed-capacity grouped-expert bodies in a native plan, retaining input ownership and route-ordered output. Requires the matching prepared-group Bridge ABI. | `true` in the dedicated entrypoint; otherwise `false` |
...
| `VLLM_HPU_DSV41_PREFILL_HYBRID_ROWS` | Occupancy-based expert routing with prepared-plan prefix replay. BF16 uses 64/128/192/256-row buckets and splits larger experts into bounded 256-row slabs; dual-FP8 modes retain 128-row first blocks and 64-row overflow blocks. Requires native route-output plans, fast dequantization and empty-block masking. Applies to every positive prompt bucket. | `true` in the dedicated entrypoint; otherwise `false` |
| `VLLM_HPU_DSV41_PREFILL_FAST_DEQUANT` | Uses the exact Prefill BF16 lookup/scale decoder on qualified normal-scale expert weights. Preserves the general decoder for other scale encodings. | `true` in the dedicated entrypoint; otherwise `false` |
