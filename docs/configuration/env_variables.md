...
| `VLLM_HPU_QWEN3_COMPILE_LAYER_GROUP_SIZE` | Compiles consecutive Qwen3-Next/Qwen3.5 decoder layers as one regional graph during decode. Prefill remains per-layer. Values greater than 1 require TP=1 and non-eager execution. Decode batches above 16 use grouped execution only with the validated direct GDN state layout; other layouts retain per-layer compilation. | `1` |
