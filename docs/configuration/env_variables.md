...
| `VLLM_HPU_DSV41_PREFILL_GROUPED_FP8` | Select `w13`, `w13_dual`, `w13_dual_prequant`, `w13_single_prequant`, `w2`, or `both` for the experimental grouped FP8 expert body. `w13_single_prequant` quantizes each input token once, runs one FP8 W13 GEMM, and retains BF16 W2; its larger activation quantization error is accepted only when explicitly selected. `w13_dual_prequant` uses a high term plus residual and combines them in one W13 GEMM. Empty keeps BF16 Prefill. | empty |
...
| `VLLM_HPU_DSV41_PREFILL_HYBRID_ROWS` | Occupancy-based expert routing with prepared-plan prefix replay. BF16 uses 64/128/192/256-row buckets and splits larger experts into bounded 256-row slabs; dual-FP8 modes retain 128-row first blocks and 64-row overflow blocks. Requires native route-output plans, fast dequantization and empty-block masking. Applies to every positive prompt bucket. | `true` for N256 in the dedicated entrypoint; otherwise `false` |
