...
| `VLLM_HPU_QWEN3_MLP_CHUNKS` | Experimental whole-MLP chunking for dense Qwen3 TP2 single-sequence prefill. Pipelines gate/up, activation, down projection and async all-reduce, using `VLLM_ROW_PARALLEL_CHUNK_THRESHOLD`. Decode and batched inputs retain the original path. Requires standard reductions; incompatible with deferred all-reduce and LoRA. Disabled pending end-to-end performance qualification. | `1` (disabled) |
| `VLLM_HPU_QWEN3_BOUNDARY_CHUNKS` | Experimental attention-output/Norm/MLP pipeline for dense Qwen3 TP2 single-sequence prefill. Uses `VLLM_ROW_PARALLEL_CHUNK_THRESHOLD`; requires standard reductions, no LoRA and no layer scaling. The attention core and state updates are unchanged. This supersedes whole-MLP chunking within qualified prefill layers, but does not change the decode path. | `1` (disabled) |
| `VLLM_HPU_QWEN3_BOUNDARY_PREFETCH` | Additionally overlap completed MLP chunks with the next layer's input norm and input projections. Requires the boundary pipeline, a complete local decoder and no auxiliary hidden-state capture. The causal attention core still consumes the full ordered sequence. Experimental and disabled by default. | `false` |
