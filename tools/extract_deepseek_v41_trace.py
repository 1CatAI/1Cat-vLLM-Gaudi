...
with (open_trace(args.trace, "rb") as src,
    ...
    for event in ijson.items(src, "traceEvents.item", use_float=True):
        ...
        if event.get("cat") == "cpu_op" and (
            "Compiled Region" in name or "execute_model" in name or "prepared_moe" in name
        ...
        if not hw and event.get("cat") in ("cpu_op", "hpu_op", "user_annotation", "privateuse1_runtime"):
            host.write(json.dumps([event['ts'], event['dur'], str(event.get('pid')),
                                   str(event.get('tid')), event.get('cat'), name],
                                  separators=(',', ':')) + '\n')
        ...
        if key not in node_ids:
            ...
            nodes.append({"engine": engine, "kernel": name, "node": key[2], "recipe": rkey,
                          "original_nodes": key[4], "reported_dtype": a.get("dataType", "")})
        if hw not in hw_ids:
...
result = {"trace": str(args.trace), "trace_sha256": digest, "events": total,
          "schema_version": 2, "hardware_columns": ["ts_us", "dur_us", "lane", "node", "hw_kind"],
          "hw_event_names": hw_names, "base_time_nanoseconds": int(base[1]) if base else None,
          ...
          "host_enqueues": host_enqueues, "cpu_markers": markers, "metadata": metadata,
          "decoder_examples": examples,
          "kernel_counts": [{"engine": key[0], "kernel": key[1], "lane_events": count}
