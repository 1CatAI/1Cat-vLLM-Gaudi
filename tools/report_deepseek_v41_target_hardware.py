...
MEASURED = {'TPC': {'TPC_SPU_START_TO_SPU_HALT'},
...
def partition(named, start, end):
    ...
    for name, spans in named.items():
        for a, b in merge(spans):
            ...
            if b > a:
                events[a].append((name, 1)); events[b].append((name, -1))
...
def anchors(root, phases):
    ...
    for row in phases['records']:
        ...
        if any(device[name]['status'] != 'complete' for name in ('stage_model_start','stage_target_done')):
            continue
        out[row['generation']] = dict(
            start_us=device['stage_model_start']['device_timestamp_ns']/1000+shift,
            end_us=device['stage_target_done']['device_timestamp_ns']/1000+shift,
            clock_width_us=hi-lo+(c['offset_high_ns']-c['offset_low_ns'])/1000,
            host_to_trace_us=(lo+hi)/2, record=row)
    return out
...
def classify(node, contract=None):
    ...
    if kernel.startswith('fused_kernel') and origin:
        ...
        for source in origin:
            ...
            if source.get('semantic_group'):
                ...
                roles.add('Router' if any(384 in (t.get('shape') or []) for t in ins+outs)
                          else '专家解码与计算')
            elif '/layers/' in sn:
                roles.add('mHC/Norm')
            else:
                roles.add('输入与尾部')
        ops = list(dict.fromkeys(n['op'] for n in origin if n['op'] not in
                   ('Reshape','ExpandDims','Slice','Squeeze','StaticReshape')))
        if len(roles) == 1:
    ...
    if 'mxfp4' in text:
        if 'dequant' in kernel:
            stage = 'W13' if any(5120 in (t.get('shape') or []) and 2304 in (t.get('shape') or []) for t in outs) else 'W2'
    ...
    if any(x in text for x in ('paged_attention','sparse_attn','pack_fp4','pack_swa','rope','kv_cache','compressor')):
    ...
    if '/layers/' in name and '/moe/' not in name and '/attention/' not in name:
        ...
        if any((t.get('shape') or [])[-1:] in ([24],[4]) for t in ins+outs):
...
def main():
    ...
    p.add_argument('run',type=Path);p.add_argument('--analysis',type=Path)
    p.add_argument('--output',type=Path);p.add_argument('--discard',type=int,default=2)
    p.add_argument('--take',type=int,default=3)
    args=p.parse_args();run=args.run.resolve();analysis=args.analysis or run/'trace-analysis'
    out=args.output or run/'target-hardware-breakdown';out.mkdir(exist_ok=False)
    ...
    if len(selected)!=args.take:raise ValueError(f'Insufficient paired C6 windows: {common}')
    summary={'scope':'C6 Target stage_model_start → stage_target_done; excludes head/prefix/draft/commit',
             'units':'ms', 'generations':selected,'discarded':common[:args.discard],
             'acquisition':str(run),'ranks':[],'pairs':[]}
    all_detail=[]
    for rank in range(4):
        inv=inventories[rank]; nodes=inv['nodes']; hw=inv['hw_event_names']
        ...
        for ni,node in enumerate(nodes):
            ...
            if matches and all(c.get('inputs')==matches[0].get('inputs') and c.get('outputs')==matches[0].get('outputs') for c in matches):
                contracts[ni]=matches[0]
        kinds={ni:classify(node,contracts.get(ni)) for ni,node in enumerate(nodes)}
        lane_names={str(m['tid']):m['args']['name'] for m in inv['metadata'] if m.get('name')=='thread_name' and m.get('pid')==0}
        samples={g:dict(groups=collections.defaultdict(list), engines=collections.defaultdict(list),
                        lanes=collections.defaultdict(list), details=collections.defaultdict(list)) for g in selected}
        with gzip.open(analysis/f'rank{rank}/hardware.jsonl.gz','rt') as f:
            for line in f:
                ts,dur,lane,ni,hi=json.loads(line); engine=nodes[ni]['engine']
                if hw[hi] not in MEASURED.get(engine,set()):continue
                for gen,s in samples.items():
                    w=windows[rank][gen];a,b=max(ts,w['start_us']),min(ts+dur,w['end_us'])
                    if b<=a:continue
                    s['groups'][kinds[ni][0]].append((a,b));s['engines'][engine].append((a,b))
                    s['lanes'][(engine,lane)].append((a,b));s['details'][ni].append((a,b,lane,hi,ts,dur))
        total_window=sum(windows[rank][g]['end_us']-windows[rank][g]['start_us'] for g in selected)
        groups=collections.Counter();exclusive=collections.Counter();engines=collections.Counter();lane_times=collections.Counter();engine_partition=collections.Counter();details=collections.defaultdict(list)
        for g,s in samples.items():
            w=windows[rank][g]
            for name,spans in s['groups'].items():groups[name]+=duration(spans)
            ...
            for key,value in partition(s['engines'],w['start_us'],w['end_us']).items():engine_partition[' + '.join(key) or 'uncovered']+=value
            for name,spans in s['engines'].items():engines[name]+=duration(spans)
            for lane,spans in s['lanes'].items():lane_times[lane]+=duration(spans)
            for ni,events in s['details'].items():details[ni].append((g,events))
        for ni,observations in details.items():
            node=nodes[ni];contract=contracts.get(ni)
            activity=sum(duration([(e[0],e[1]) for e in events]) for g,events in observations)
            detail=dict(rank=rank,group=kinds[ni][0],purpose=kinds[ni][1],**node,
                        activity_ms_per_C6=activity/len(selected)/1000,share_pct=activity/total_window*100,
                        lane_packets=sum(len(events) for g,events in observations),
                        physical_calls=None,mean_call_ms=None,physical_call_note='Not inferred from lane packet count',
                        contract=contract,observations=[dict(generation=g,events=events) for g,events in observations])
            all_detail.append(detail)
        def rows(counter):return [dict(name=name,ms_per_C6=value/len(selected)/1000,share_pct=value/total_window*100) for name,value in counter.most_common()]
        result=dict(rank=rank,stage_window_ms=total_window/len(selected)/1000,
                    max_clock_uncertainty_ms=max(windows[rank][g]['clock_width_us'] for g in selected)/1000,
                    groups=rows(groups),exclusive=rows(exclusive),engines=rows(engines),engine_partition=rows(engine_partition),
                    lanes=[dict(engine=e,lane=l,name=lane_names.get(l,l),active_ms_per_C6=v/len(selected)/1000,duty_pct=v/total_window*100) for (e,l),v in sorted(lane_times.items())],
                    kernel_rows=len(details),matched_contract_rows=sum(ni in contracts for ni in details))
        summary['ranks'].append(result)
    for tp in range(2):
        paired=[]
        for g in selected:
            a,b=windows[tp][g],windows[tp+2][g]
            paired.append(dict(generation=g,pp0_ms=(a['end_us']-a['start_us'])/1000,
                               handoff_ms=(b['start_us']-a['end_us'])/1000,
                               pp1_ms=(b['end_us']-b['start_us'])/1000,
                               total_ms=(b['end_us']-a['start_us'])/1000))
        summary['pairs'].append(dict(tp=tp,samples=paired,mean={key:statistics.mean(r[key] for r in paired) for key in ('pp0_ms','handoff_ms','pp1_ms','total_ms')}))
    (out/'summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n')
    with gzip.open(out/'kernel-details.json.gz','wt') as f:json.dump(all_detail,f,ensure_ascii=False)
    with (out/'kernel-details.csv').open('w') as f:
        fields=['rank','group','purpose','engine','kernel','node','recipe','reported_dtype','activity_ms_per_C6','share_pct','lane_packets','physical_calls','mean_call_ms']
        writer=csv.DictWriter(f,fieldnames=fields,extrasaction='ignore');writer.writeheader();writer.writerows(all_detail)
    (out/'selected-windows.json').write_text(json.dumps({rank:{g:windows[rank][g] for g in selected} for rank in range(4)},indent=2)+'\n')
