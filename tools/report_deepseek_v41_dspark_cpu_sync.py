...
def scan(path):
    ...
    phases = {"verify_and_commit": [], "draft_propose": [],
              "target_decode_C6": [], "target_decode_C1": []}
    candidates = []
    total = 0
    with opener(path, "rb") as stream:
        for event in ijson.items(stream, "traceEvents.item", use_float=True):
            ...
            if event.get("cat") != "cpu_op":
                continue
            if name not in {"aten::to", "aten::_to_copy", "aten::copy_", "aten::argmax",
                            "aten::tolist", "aten::item"}:
                continue
            args = event.get("args") or {}
            candidates.append({"name": name, "ts": ts, "dur": dur,
    ...
    for phase, spans in phases.items():
        ...
        by_name = {}
        for row in selected:
            key = (row["name"], json.dumps(row["input_dims"], sort_keys=True),
                   json.dumps(row["input_type"], sort_keys=True))
            by_name.setdefault(key, []).append(row)
        groups = []
        for (name, dims, types), values in by_name.items():
            groups.append({"kernel": name, "input_dims": json.loads(dims),
                           "input_type": json.loads(types), "calls": len(values),
        ...
        rows.append({"phase": phase, "calls": len(spans),
...
def main():
    ...
    print(json.dumps({"output": str(args.output), "ranks": [
        {"trace": r["trace"], "phases": r["phases"]} for r in payload["ranks"]]},
