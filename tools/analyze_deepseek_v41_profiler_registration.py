...
def analyze(path):
    ...
    for number, line in enumerate(path.read_text(errors="replace").splitlines(), 1):
        ...
        if kind == "enter":
            if active:
                findings.append({"line": number, "kind": "overlapping registration calls",
                                 "entered": key, "already_active": sorted(active)})
            active.add(key)
            ...
            if not bucket_ptr or not buckets or buckets > 1000000 or count > 8190 or bool(first) != bool(count):
                findings.append({"line": number, "kind": "implausible SDK unordered-map control words",
                                 "call": key, "map": calls[key]["map"]})
        elif kind == "debug":
            ...
            calls[key].setdefault("nodes", []).append({"index": int(fields[2]), "name": fields[3],
                                                       "operation": fields[4], "dtype": fields[5]})
    ids = [call["debug"]["recipe"] for call in calls.values() if "debug" in call]
    result = {"source": str(path.resolve()), "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
              "registrations": len(calls), "maximum_simultaneous_calls": maximum,
              "unfinished_calls": {key: calls[key] for key in active}, "findings": findings,
              "unique_debug_recipe_ids": len(set(ids)),
              "duplicate_id_frequencies": {key: value for key, value in collections.Counter(ids).items() if value > 1},
    ...
    print(json.dumps({key: value for key, value in result.items() if key not in ("unfinished_calls", "findings")}),
          flush=True)
    return result
