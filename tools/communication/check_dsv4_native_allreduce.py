...
def main():
    ...
    def program(x):
        partial1 = x + 2
        first = torch.ops.vllm_gaudi.tp2_allreduce_plain(partial1)
        partial2 = x + 4 if args.parallel_reductions else first + 2
        second = torch.ops.vllm_gaudi.tp2_allreduce_plain(partial2)
    ...
    topology = DecoderTopology("deepseek_v4", (1,), 2, False)
    ...
    for step in range(34):
        ...
        if args.compiled_embedding:
            ...
            x = torch.full((1, 4096), float(step + rank), dtype=torch.bfloat16).to("hpu")
        ...
        if result is None:
            with collect_prepared_group_replays(owner=owner, adapter=topology,
                                               snapshot=lambda: SimpleNamespace(restore=lambda: None), **roots):
                output = compiled(x)
    ...
    (args.output / f"rank{rank}.json").write_text(json.dumps(dict(status="exact", steps=34, **stats), indent=2))
