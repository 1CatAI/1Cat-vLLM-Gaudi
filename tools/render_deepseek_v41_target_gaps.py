...
def analyze_phase_host(root, rank, phases):
    ...
    for phase, spans in named.items():
        ...
        for kind, intervals in hits[phase].items():
            ...
            for a, b in merged:
                boundaries[a].append((kind, 1))
                boundaries[b].append((kind, -1))
        ...
        results[phase] = {'gap_ms': expected, 'host_unions_ms': unions, 'exclusive_host_observation_ms': dict(ledger)}
    return results
...
def main():
            ...
            for phase, value in summary[str(rank)]['phase_ms'].items():
                ...
                rows.append(
                    dict(tp_pair=pair,
                         ...
                         label=LABELS[phase],
                         gap_ms=value,
                         share_of_gap_pct=100 * value / total_gap,
    ...
    report = [
        '# DSpark C6：122.892677 ms 未覆盖区间继续拆解', '',
    ...
    for pair in (0, 1):
        ...
        for row in selected:
            report.append(
                f"| {row['label']} | {row['gap_ms']:.6f} | {row['share_of_gap_pct']:.3f}% | {row['share_of_target_pct']:.3f}% |"
            )
        report += [
            f"| **合计** | **{sum(r['gap_ms'] for r in selected):.6f}** | **100%** | **{sum(r['share_of_target_pct'] for r in selected):.3f}%** |",
            ''
        ]
    report += [
        '## 大项内部：同区间观测继续去重', '',
    ...
    report += [
        ...
        '- 本采集带 profiler、阶段诊断和计划转储，不能用于 <50 ms 无 profiler 验收。该目标仍未达到。', '', '## 边界及来源', '',
    ...
    sources = out / 'analysis-source'
    sources.mkdir(exist_ok=True)
    for name in ('report_deepseek_v41_target_gaps.py', 'render_deepseek_v41_target_gaps.py',
                 'report_deepseek_v41_target_hardware.py'):
