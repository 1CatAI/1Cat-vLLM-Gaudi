...
def main():
    ...
    for ratio in (1, 2):
        ...
        hp_q, hp_w, hp_cache, hp_pages, hp_candidates = (
            x.to("hpu") for x in (q, weights, packed, pages, candidates))
        position = torch.zeros(1, dtype=torch.int32, device="hpu")

        def select(q, w, cache, table, pos, candidates, ratio=ratio):
            return runtime_index_select(q, w, cache, table, pos, candidates,
                                        ratio=ratio, capacity=capacity, reindex=args.reindex,
        ...
        for visible in map(int, args.lengths.split(",")):
            ...
            if visible <= 512:
                ...
                scores, _ = torch.ops.custom_op.custom_deepseek_v41_index_scores_gaudi2(
                    hp_q, hp_w, hp_cache, hp_pages, position, hp_candidates, ratio,
                    int(args.reindex), 16384 if args.reindex else capacity)
            ...
            case = dict(ratio=ratio, visible=visible, wall_ms=wall_ms, selected_equal=equal,
                        blocks_equal=blocks_equal, scores_equal=score_equal, max_score_error=max_error,
                        compilation_before=before, compilation_after=after,
                        reindex=args.reindex, first_selected=selected[0, :12].tolist(),
                        first_blocks=blocks[0, :12].tolist() if blocks is not None else None)
            report["cases"].append(case)
            print(json.dumps(case), flush=True)
            save()
            if not (equal and blocks_equal and score_equal):
                torch.save(dict(query=q, weights=weights, keys=keys, selected=selected,
                                expected=expected, blocks=blocks, expected_blocks=expected_blocks),
