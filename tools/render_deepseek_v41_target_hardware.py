...
def calls(row):
    ...
    count = 0
    if not c:
        return None, None, 'no compiled compute contract', []
    for obs in row['observations']:
        es = sorted(obs['events'], key=lambda e: e[4])
        cohorts = []
        if row['engine'] == 'TPC':
            expected = sum(c['symbol']['working_engines'])
            if not expected or len(es) % expected:
                return None, None, 'ROI packet count incomplete', []
            cohorts = [es[i:i + expected] for i in range(0, len(es), expected)]
        elif row['engine'] == 'MME':
            lanes = collections.defaultdict(list)
            for e in es:
                lanes[(e[2], e[3])].append(e)
            if len({len(v) for v in lanes.values()}) != 1:
                return None, None, 'MME lane counts differ', []
            cohorts = [[v[i] for v in lanes.values()] for i in range(len(next(iter(lanes.values()))))]
        else:
            return None, None, 'DMA packets are reported separately', []
        ...
        out.extend((b - a) / 1000 for a, b in spans)
        count += len(spans)
    note = ('compiled TPC node, all recipe ROIs and engine packets'
            if row['engine'] == 'TPC' else 'MME monitor cohort, equal ordered lane counts, nonoverlapping cohorts')
...
def main():
    ...
    raw = run / 'target-hardware-breakdown'
    out = args.output or run / 'target-kernel-report'
    out.mkdir(exist_ok=False)
    summary = json.loads((raw / 'summary.json').read_text())
    windows = json.loads((raw / 'selected-windows.json').read_text())
    with gzip.open(raw / 'kernel-details.json.gz', 'rt') as f:
        data = json.load(f)
    n = len(summary['generations'])
    contracts = {}
    ...
    aggregated = {}
    lite = []
    decoded_sram = collections.Counter()
    pergen_calls = {rank: {g: collections.Counter() for g in summary['generations']} for rank in range(4)}
    for index, row in enumerate(data):
        rank = row['rank']
        c = contracts[rank].get((row['recipe'].split(':')[0], row['node']))
        if c:
            row['contract'] = c
            match[(rank, row['engine'], 'matched')] += 1
        else:
            match[(rank, row['engine'], 'unmatched')] += 1
        group, purpose = classify(row, c)
        ...
        for obs in row['observations']:
            ...
            if row['engine'] == 'TPC' and c:
                ...
                if expected and len(es) % expected == 0:
                    ...
                    if label:
                        pergen_calls[rank][gen][label] += len(es) // expected
        if c and 'mxfp4_prepared_dequant' in row['kernel']:
            for t in c['outputs']:
                decoded_sram[(rank, t['dtype'], t['location'])] += 1
        ...
        if key not in aggregated:
            aggregated[key] = dict(rank=rank,
                                   ...
                                   node_refs=[])
        a = aggregated[key]
        a['kernels'].add(row['kernel'])
        a['lane_packets'] += row['lane_packets']
        if count is None:
            a['unknown_calls'] = True
        else:
            a['call_count'] += count
            a['call_latencies'].extend(latencies)
        for obs in row['observations']:
            a['spans'][obs['generation']].extend((e[0], e[1]) for e in obs['events'])
    ...
    for rank, r in enumerate(summary['ranks']):
        ...
        for gen, named in spans[rank].items():
            w = windows[str(rank)][str(gen)]
            a, b = w['start_us'], w['end_us']
            for group, s in named.items():
                groups[group] += duration(s)
        ...
        def rows(counter, total=total):
            return [dict(name=k, ms_per_C6=v / n / 1000, share_pct=v / total * 100) for k, v in counter.most_common()]

        r['groups'] = rows(groups)
        r['exclusive'] = rows(exclusive)
        r['per_generation'] = sample
        r['matched_compute_nodes'] = sum(v for (rk, e, status), v in match.items()
                                         if rk == rank and e != 'DMA' and status == 'matched')
        r['unmatched_compute_nodes'] = sum(v for (rk, e, status), v in match.items()
                                           if rk == rank and e != 'DMA' and status == 'unmatched')
        ...
        for engine in ('TPC', 'MME', 'DMA'):
            ...
            r['monitor_duty'][engine] = dict(observed_lanes=len(ls),
                                             mean_observed_lane_duty_pct=sum(x['active_ms_per_C6'] for x in ls) /
                                             len(ls) / r['stage_window_ms'] * 100 if ls else None)
    kernel_rows = []
    for key, a in aggregated.items():
        active = sum(duration(s) for s in a.pop('spans').values()) / n / 1000
        c = a.pop('contract')
        lat = a.pop('call_latencies')
        unknown = a.pop('unknown_calls')
        count = a.pop('call_count')
        a['kernels'] = sorted(a['kernels'])
        a.update(activity_ms_per_C6=active,
                 stage_share_pct=active / summary['ranks'][a['rank']]['stage_window_ms'] * 100,
                 calls_per_C6=None if unknown else count / n,
                 observed_calls=None if unknown else count,
    ...
    for rank, r in enumerate(summary['ranks']):
        ...
        with gzip.open(run / f'trace-analysis/rank{rank}/host.jsonl.gz', 'rt') as f:
            for line in f:
                ...
                if ('Torch-Compiled Region' in name or name == 'compileGraph'
                        or name.startswith('enqueueWithExternalEvents') or name.startswith('v41::')):
                    events.append((ts, ts + d, cat, name))
        for gen in summary['generations']:
            w = windows[str(rank)][str(gen)]
            h = w['record']['host']
            offset = w['host_to_trace_us']
            a = h['round_start'] / 1000 + offset
            b = h['target_submit_done'] / 1000 + offset
            ...
            host_summary.append(
                dict(generation=gen,
                     compiled_group_calls=len(compiled),
                     compiled_group_cpu_union_ms=duration((e[0], e[1]) for e in compiled) / 1000,
                     compileGraph_calls=len(compiles),
                     compileGraph_cpu_union_ms=duration((e[0], e[1]) for e in compiles) / 1000,
    ...
    summary[
        'count_contract'] = 'TPC: serialized recipe ROI engine totals, ordered full-node cohorts. MME: observed equal-count ordered monitor cohorts. Incomplete/overlapping cohorts retain null; packet counts are separate.'
    summary['source_scripts'] = {
        p.name: hashlib.sha256(p.read_bytes()).hexdigest()
        for p in (Path(__file__), Path(__file__).with_name('report_deepseek_v41_target_hardware.py'),
                  Path(__file__).with_name('enrich_deepseek_v41_target_contracts.py'))
    }
    (out / 'summary.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n')
    (out / 'kernel-summary.json').write_text(json.dumps(kernel_rows, ensure_ascii=False, indent=2) + '\n')
    with gzip.open(out / 'node-details.json.gz', 'wt') as f:
        json.dump(lite, f, ensure_ascii=False)
    with (out / 'kernel-summary.csv').open('w') as f:
        fields = [
            'rank', 'group', 'purpose', 'engine', 'kernel', 'activity_ms_per_C6', 'stage_share_pct', 'calls_per_C6',
            'observed_calls', 'mean_call_ms', 'lane_packets', 'io_contract', 'graph'
        ]
        w = csv.DictWriter(f, fieldnames=fields, extrasaction='ignore')
        w.writeheader()
        w.writerows(kernel_rows)
    md = [
        '# DSpark C6 Target：功能与显卡资源拆解', '',
    ...
    for pair in summary['pairs']:
        ...
        pair['functional_bill'] = [
            dict(group=k, exclusive_ms=v, share_pct=v / total * 100, activity_union_ms=un.get(k))
            for k, v in ex.most_common()
        ]
        md += [
            f'## TP{tp} 完整 Target：{total:.6f} ms/C6', '', '| 功能组 | 互斥 ms/C6 | 占完整 Target | 活动并集 ms/C6（可重叠） |',
            '|---|---:|---:|---:|'
        ]
    ...
    md += [
        '## 四卡资源占用', '', '活动率表示窗口内该引擎至少一个监测 lane 活动的时间并集；引擎间可重叠。lane 平均值仅为监测点占空比，不是峰值 FLOPS 或 HBM 带宽效率。', '',
        '| rank | stage ms/C6 | TPC 活动 ms / % | MME 活动 ms / % | DMA 活动 ms / % | 三引擎未覆盖 ms |',
        '|---:|---:|---:|---:|---:|---:|'
    ]
    for r in summary['ranks']:
        e = {x['name']: x for x in r['engines']}
        gap = next(x['ms_per_C6'] for x in r['exclusive'] if x['name'] == 'TPC/MME/DMA 未覆盖区间')

        def fmt(key, engines=e):
            return f"{engines[key]['ms_per_C6']:.6f} / {engines[key]['share_pct']:.3f}%"

        md.append(
            f"| {r['rank']} | {r['stage_window_ms']:.6f} | {fmt('TPC')} | {fmt('MME')} | {fmt('DMA')} | {gap:.6f} |")
    md += [
        '', '## 调用与数据流审计', '',
    ...
    for r in summary['ranks']:
        md += [
            f"rank {r['rank']}: compute nodes matched {r['matched_compute_nodes']}, unresolved {r['unmatched_compute_nodes']}; TPC lanes {r['monitor_duty']['TPC']['observed_lanes']}, MME lanes {r['monitor_duty']['MME']['observed_lanes']}."
        ]
    md += [
        '', '## 逐 kernel 功能明细', '',
    ...
    for a in kernel_rows:
        io = a['io_contract']
        contract = (' ; '.join(str(t['shape']) + ' ' + t['dtype'] + ' ' + t['location'] for t in io[0]) + ' → ' +
                    ' ; '.join(str(t['shape']) + ' ' + t['dtype'] + ' ' + t['location']
                               for t in io[1])) if io else '未记录张量合同（DMA 命令/拷贝）'
        ...
        md.append(
            f"| {a['rank']} | {a['group']} / {a['purpose']} | `{a['kernel']}` | {contract} | {a['activity_ms_per_C6']:.6f} | {a['stage_share_pct']:.3f}% | {ct} | {mean} | {a['lane_packets']} |"
        )
    ...
    (out / 'TRACE.html').write_text(
        page.replace('DATA',
                     json.dumps(kernel_rows, ensure_ascii=False).replace('</', '<\\/')))
    print(json.dumps(dict(output=str(out), rows=len(kernel_rows), pairs=[p['mean'] for p in summary['pairs']]),
                     ensure_ascii=False),
          flush=True)
