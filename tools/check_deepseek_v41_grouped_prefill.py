...
def main():
    ...
    import vllm_gaudi.ops.deepseek_v41_grouped_prefill as grouped_prefill
    ...
    def load(projection):
        ...
        for index in range(args.experts):
            ...
            qs.append(pq); ss.append(ps); cs.append(pc)
    ...
    if args.eager_body:
        grouped_prefill.compiled_body = lambda _signature: grouped_prefill.grouped_body
        grouped_prefill.compiled_reduce = lambda _signature: grouped_prefill.ordered_reduce

        def destination_body(slots, sentinel):
            return torch.where(slots >= 0, slots, sentinel).flatten()

        grouped_prefill.compiled_destinations = lambda _signature: destination_body
    ...
    reserve = None
    if args.reserve_gib:
        reserve = torch.empty(int(args.reserve_gib * (1 << 30)), dtype=torch.uint8, device="hpu")
        reserve.zero_()
    ...
    report = {
        ...
        "eager_body": args.eager_body,
        "reserve_bytes": 0 if reserve is None else reserve.numel(),
    }
