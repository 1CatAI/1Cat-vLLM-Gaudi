...
def process_snapshot(process_record):
    ...
    for process in processes:
        try:
            ...
            rows.append({"pid": process.pid, "name": process.name(), "created": process.create_time(),
                         "user_ms": cpu.user * 1000, "system_ms": cpu.system * 1000,
...
def main():
    ...
    profiling.add_argument("--external-profile", action="store_true",
    ...
    parser.add_argument("--thinking-mode", choices=("default", "chat", "thinking"), default="default",
    ...
    body = {"model": args.model, "messages": [{"role": "user", "content": content}],
            "temperature": 0, "max_tokens": args.tokens, "ignore_eos": not args.stop_at_eos,
    ...
    request_record = {"started_at": datetime.now(timezone.utc).isoformat(), "url": args.url,
                      ...
                      "max_tokens": args.tokens, "temperature": 0, "ignore_eos": not args.stop_at_eos,
    ...
    try:
        ...
        report = {"request_ms": (finished - started) / 1e6, "ttft_ms": chunks[0] if chunks else None,
                  ...
                  "timing_note": "DSpark may emit multiple tokens per chunk; arrivals measure the API, "
            ...
            try:
                ...
                for row in resource_after["processes"]:
                    ...
                    if previous is not None:
                        deltas.append({"pid": row["pid"], "name": row["name"],
                                       "user_ms": row["user_ms"] - previous["user_ms"],
                                       "system_ms": row["system_ms"] - previous["system_ms"]})
                resources = {"before": resource_before, "after": resource_after, "cpu_delta": deltas,
                             "note": "Host CPU time summed across process threads; snapshots bracket the API request. "
        ...
        if args.profile:
            try:
                ...
                (args.output / "profile.json").write_text(json.dumps({
                    "status": "stop_failed", "error": repr(exc), "request_error": error}) + "\n")
                if error is None:
                    session.close()
                    raise
        session.close()
