...
def main():
    ...
    def chain(query, weights, packed, table, position, pool, main, swa, attention_query):
        if args.reference:
            ...
            selected, _ = runtime_index_select(query,
                                               weights,
                                               packed,
                                               table,
                                               position,
        ...
        result = torch.ops.custom_op.custom_deepseek_v41_paged_mla_mme_gaudi2(attention_query, swa, main, row_ids,
                                                                              indices, sink, scale, lengths)
    ...
    report = dict(capacity=capacity,
                  visible=visible,
                  reference=args.reference,
                  native_replay=args.native_replay,
                  recipes=None if replay is None else replay.recipes,
                  warm_ms=warm_ms,
                  wall_ms=wall,
                  device_ms=device,
                  wall_median_ms=statistics.median(wall),
                  device_median_ms=statistics.median(device),
                  output_finite=bool(result[0].cpu().isfinite().all()),
