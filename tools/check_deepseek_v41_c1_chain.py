...
"""Isolate C1 state reset, native replay and ordinary TP output consumers."""
...
def main():
    ...
    native = torch.compile(owner.native_forward, backend="hpu_backend", fullgraph=True, dynamic=False)
    topology = DecoderTopology("deepseek_v41_pp1", (4,), 2, False)
    outputs = []
    profiler = None
    sequence = (6, 1, 1, 1, 1, 6, 1, 1, 1, 1) if args.profile else (6, 1, 1, 1, 1, 6)
    for step, count in enumerate(sequence):
        if args.profile and step == 7:
            torch.hpu.synchronize()
            profiler = torch.profiler.profile(
                activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.HPU],
                record_shapes=True, with_stack=False,
                on_trace_ready=torch.profiler.tensorboard_trace_handler(str(evidence / "traces"), use_gzip=True))
            print(f"RANK {rank}: starting hardware profiler and native recapture", flush=True)
            profiler.start()
        ...
        if count == 1 and step not in (6, 7):
    ...
    (evidence / f"rank{rank}-result.json").write_text(json.dumps({
        ...
        "reset_state": args.reset_state, "sync_native": args.sync_native, "outputs": outputs,
