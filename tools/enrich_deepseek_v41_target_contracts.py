...
def io(node,prefix):
    return [tensor(value) for key,value in sorted(node['attrs'].items(),key=lambda x:int(x[0].rsplit(':',1)[1]) if x[0].startswith(prefix) else -1) if key.startswith(prefix)]
...
def load_graph(path):
    nodes=graph_nodes(path);alias={};producers={}
    for n in nodes:
        for t in io(n,'inputTensor:')+io(n,'outputTensor:'):
            if t['alias']:alias[t['name']]=t['alias']
        for t in io(n,'outputTensor:'):producers[t['name']]=n
    pre=path.with_name(path.name.replace('-PostGraph-','-PreGraph-'))
    original=graph_nodes(pre) if pre.exists() else []
    consumers=collections.defaultdict(list)
    for n in original:
        for t in io(n,'inputTensor:'):consumers[t['name']].append(n)
    engram=set();pending=[]
    for n in original:
        if any(t['shape']==[25600,6144] for t in io(n,'inputTensor:')):
            engram.add(n['name']);pending.extend(t['name'] for t in io(n,'outputTensor:'))
    visited=set()
    while pending:
        tname=pending.pop()
        if tname in visited:continue
        visited.add(tname)
        for n in consumers[tname]:
            ...
            engram.add(n['name'])
            if n['op']=='cast_f32_to_bf16' and any(t['shape'][-2:]==[4,5120] for t in outputs):continue
            if n['op'].startswith('linear_fwd'):continue
            pending.extend(t['name'] for t in outputs)
    return dict(nodes=nodes,alias=alias,producers=producers,pre=original,
                engram=engram,pre_producers={t['name']:n for n in original for t in io(n,'outputTensor:')})


def roots(g,name):
    result=set()
    while name not in result:
        result.add(name);short=base_name(name);result.add(short)
        if name in g['alias']:name=g['alias'][name]
        elif short!=name:name=short
        else:break
    return result
...
def expression(g,c):
    ...
    while pending:
        name=pending.pop()
        if name in visited or name in inputs:continue
        visited.add(name);n=g['pre_producers'].get(name)
        if n is None:continue
        if (n['op'] in ('GEMM','BatchGemm')
                or n['op'].startswith(('custom_', 'linear_fwd', 'batch_gemm'))):
            return [],'unresolved_boundary'
        found[n['name']]=n
        for t in io(n,'inputTensor:'):
            if not roots(g,t['name']) & inputs:pending.append(t['name'])
...
def main():
    ...
    cache={}
    for rank in range(4):
        path=args.metadata/f'rank{rank}/node-contracts.json';data=json.loads(path.read_text());counts=collections.Counter()
        for c in data:
            if not c['matched']:continue
            gp=Path(c['graph']['path'])
            if gp not in cache:cache[gp]=load_graph(gp)
            graph=cache[gp]
            if c['symbol']['kernel'].startswith('fused_kernel'):
                ns,method=expression(graph,c)
                c['fused_original_nodes']=[dict(name=n['name'],op=n['op'],
                    semantic_group='Engram' if n['name'] in graph['engram'] else None) for n in ns]
                c['origin_method']=method;counts[method]+=1
