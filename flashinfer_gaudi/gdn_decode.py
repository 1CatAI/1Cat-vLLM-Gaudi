...
def _call_native_packed(
    ...
    # The first production GUID uses FP32 operands and takes exp(g). Keeping
    # this compatibility call isolated lets a BF16-input GUID replace it
    # without changing the public Python API.
    decay = torch.exp(log_decay.reshape(packed_qkv.shape[0], -1).to(torch.float32)).contiguous()
    beta_f32 = beta.reshape(packed_qkv.shape[0], -1).to(torch.float32).contiguous()
    packed_f32 = packed_qkv.to(torch.float32).contiguous()
    result = op(state_pool, packed_f32, decay, beta_f32, state_indices.reshape(-1).to(torch.int32))
    if not isinstance(result, tuple) or len(result) != 2:
        raise RuntimeError("Gaudi packed GDN op must return (updated_state_rows, output).")
...
def gated_delta_rule_decode_packed(
    ...
    if policy == "bridge" or (policy == "auto" and bridge_auto_enabled()):
        ...
        if supported and bridge_op is not None:
            return _call_native_packed(
                bridge_op, packed_qkv, log_decay, beta, state_pool, load_state_indices
            ), state_pool
        if policy == "bridge":
    ...
    if use_public and supported and public_op is not None:
        return _call_native_packed(
            public_op, packed_qkv, log_decay, beta, state_pool, load_state_indices
        ), state_pool
    if policy == "public":
...
def gated_delta_rule_decode(
    ...
    initial_state_indices: torch.Tensor | None = None,
    output_state_indices: torch.Tensor | None = None,
    state_layout: Literal["VK", "KV"] = "VK",
) -> tuple[torch.Tensor, torch.Tensor]:
    """Unified decode facade; the optimized first milestone supports VK."""
    if state_layout != "VK":
        raise NotImplementedError("Gaudi GDN decode currently supports only VK/K-last recurrent state layout.")
